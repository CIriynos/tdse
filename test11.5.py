...
bound_state_indices = [i for i in range(len(eigen_waves) - 41 + 10)]
...
pos1_py, _ = get_pos_expect_spectrum_1d(laser.get_ts(), dipole_values, max_k=(n_cut_off_estim + 20) * omega0)
...
pos1, _ = get_pos_expect_spectrum_1d(laser.get_ts(), pos, max_k=(n_cut_off_estim + 20) * omega0)
ax.plot(ks / laser.omega0, np.pow(ks, 3) * np.pow(np.abs(pos1_py - pos1), 2), label="diff", linestyle='dashed')

plt.xticks(range(1, (n_cut_off_estim), 2))
plt.grid(True, alpha=0.2)
plt.yscale('log')
plt.ylim(1e-18, 1e2)
plt.legend()
