...
# tdse-hg
accel, pos, accel_free, pos_free, accel_bound, pos_bound, pos_cross, accel_cross, \
sampling_points_collect, sampling_points_collect_bound, \
sampling_points_collect_free, sampling_points_collect_cross, sampling_positions = tdse_fd1d_hg_analytical(world, buffer, wave, light_field=laser_all, Xi=Xi, bound_states=eigen_waves, dU_data=Vx(xs))
...
max_display_n = EXPORT(50 + 20, "max_display_n");
...
# accel
accel_expect_spectrum, ks = get_pos_expect_spectrum_1d(laser.get_ts(), accel, max_k= max_display_n * omega0);
accel_expect_spectrum_free, _ = get_pos_expect_spectrum_1d(laser.get_ts(), accel_free, max_k= max_display_n * omega0);
accel_expect_spectrum_bound, _ = get_pos_expect_spectrum_1d(laser.get_ts(), accel_bound, max_k= max_display_n * omega0);
accel_expect_spectrum_residual = accel_expect_spectrum - accel_expect_spectrum_free - accel_expect_spectrum_bound;
accel_expect_spectrum_cross, _ = get_pos_expect_spectrum_1d(laser.get_ts(), accel_cross, max_k= max_display_n * omega0);

plt.figure()
plt.plot(ks / omega0, (1.0 / ks) * np.pow(np.abs(accel_expect_spectrum), 2), label="accel expect total", lw = 2.0);
plt.plot(ks / omega0, (1.0 / ks) * np.pow(np.abs(accel_expect_spectrum_free), 2), label="accel expect free", lw = 1.5);
plt.plot(ks / omega0, (1.0 / ks) * np.pow(np.abs(accel_expect_spectrum_bound), 2), label="accel expect bound", lw = 1.5);
# plt.plot(ks / omega0, (1.0 / ks) * np.pow(np.abs(pos_expect_spectrum_residual), 2), label="accel expect residual", lw = 1.5)
plt.plot(ks / omega0, (1.0 / ks) * np.pow(np.abs(accel_expect_spectrum_cross), 2), label="accel expect cross", lw = 1.5);
...
# get sampling points spectrum
probe_freq = omega0 * (2)
probe_freq_id = math.floor(probe_freq / (ks[1] - ks[0]))
sampling_points_spectrum = np.zeros(len(sampling_positions), dtype=complex)
sampling_points_spectrum_bound = np.zeros(len(sampling_positions), dtype=complex)
sampling_points_spectrum_free = np.zeros(len(sampling_positions), dtype=complex)
sampling_points_spectrum_cross = np.zeros(len(sampling_positions), dtype=complex)
sampling_points_spectrum_recover = np.zeros(len(sampling_positions), dtype=complex)
windows_data = np.hanning(len(sampling_points_collect[0]))
# windows_data = np.ones(len(sampling_points_collect[0]))

for (i, x_pos) in enumerate(sampling_positions):
    sampling_points_spectrum[i] = np.fft.fft(sampling_points_collect[i] * windows_data)[probe_freq_id]
    sampling_points_spectrum_bound[i] = np.fft.fft(sampling_points_collect_bound[i] * windows_data)[probe_freq_id]
    sampling_points_spectrum_free[i] = np.fft.fft(sampling_points_collect_free[i] * windows_data)[probe_freq_id]
    sampling_points_spectrum_cross[i] = np.fft.fft(sampling_points_collect_cross[i] * windows_data)[probe_freq_id]
    sampling_points_spectrum_recover[i] = np.fft.fft((sampling_points_collect_bound[i] + sampling_points_collect_free[i] + sampling_points_collect_cross[i]) * windows_data)[probe_freq_id]

plt.figure()
plt.plot(sampling_positions, np.real(sampling_points_spectrum), lw=2.0, label="sampling points spectrum")
plt.plot(sampling_positions, np.real(sampling_points_spectrum_free), label="sampling points spectrum free")
plt.plot(sampling_positions, np.real(sampling_points_spectrum_bound), label="sampling points spectrum bound")
plt.plot(sampling_positions, np.real(sampling_points_spectrum_cross), label="sampling points spectrum cross")
plt.legend()
plt.show()
